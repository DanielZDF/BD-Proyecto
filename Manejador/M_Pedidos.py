...
class M_Pedido(Conexion):
	def insertar_pedido(self, municipio, ciudad, n_hamburg, monto_deliv, monto_t, metodo_p, estado_d, fecha, cedula,remarks):
            ...
            try: # Trata de insertar un cliente
                cursor.execute("INSERT INTO Pedido (municipio, ciudad, cant_hamburguesas, m_delivery, m_total, metodo_pago, estado, fecha, cedula_cliente, remark) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);",[municipio, ciudad, n_hamburg, monto_deliv, monto_t, metodo_p, estado_d, fecha, cedula,remarks])
